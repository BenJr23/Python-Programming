        ...
        def vehicle_car_window_admin():
            ...
            def delete_car():
                ...
                if confirm_delete:
                    ...
                    cursor.execute("DELETE FROM CAR WHERE VehicleID=?", (vehicle_id,))
                    conn.commit()
        ...
        def vehicle_truck_window_admin():
            ...
            def delete_truck():
                ...
                if confirm_delete:
                    ...
                    cursor.execute("DELETE FROM TRUCK WHERE VehicleID=?", (vehicle_id,))
                    conn.commit()
        ...
        def vehicle_motorcycle_window_admin():
            ...
            def delete_motorcycle():
                ...
                if confirm_delete:
                    ...
                    cursor.execute("DELETE FROM MOTORCYCLE WHERE VehicleID=?", (vehicle_id,))
                    conn.commit()
...
class VehicleRentingApp(tk.Tk):
    ...
    def create_sidebar_tile(self, parent, text, bg_color):
        ...
        label.bind("<Button-1>",
                   lambda e, tile=tile, label=label, text=text, bg_color=bg_color: self.on_tile_click(tile, label, text,
                                                                                                      bg_color))
    ...
    def create_reservation_frame(self, frame):
        ...
        def edit_reservation():
            ...
            status_combobox = ttk.Combobox(form_frame,
                                           values=["PENDING/PAYMENT", "CANCELLED", "UNPAID/CANCELLED", "PAID/CONFIRMED",
                                                   "PAID/CANCELLED", "IN-PROGRESS", "COMPLETED"], state="readonly")
        ...
        def load_vehicle_new_window():
            def vehicle_car_window_admin():
                def create_reservation():
                    ...
                    status_combobox = ttk.Combobox(form_frame,
                                                   values=["PENDING/PAYMENT", "CANCELLED", "UNPAID/CANCELLED",
                                                           "PAID/CONFIRMED", "PAID/CANCELLED", "IN-PROGRESS",
                                                           "COMPLETED"],
                                                   state="readonly")
                    status_combobox.pack()
                ...
                tk.Button(button_frame, text="SELECT", command=create_reservation, bg="lightblue").pack(side='left',
                                                                                                        padx=5)
        ...
        tree = ttk.Treeview(frame, columns=('GuestNo', 'GuestName', 'VehicleType', 'VehicleName', 'Status'),
                            show='headings')
        for row in tree.get_children():
            tree.delete(row)
    ...
    def create_payment_frame(self, frame):
        ...
        dropdown = tk.OptionMenu(frame, selected_method, *payment_methods,
                                 command=lambda event=None: payment_type_change(event))
